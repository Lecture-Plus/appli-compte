// Contenu embarqué de setup/Code.gs — mis à jour manuellement si Code.gs change
export const CODE_GS = `/**
 * ================================================================
 *  Compta+ – Sync Google Drive (version multi-backups)
 *
 *  ▶ INSTRUCTIONS D'INSTALLATION :
 *  1. https://script.google.com → Nouveau projet "Compta+ Sync"
 ...
 *
 *  ▶ COMPORTEMENT :
 *  - doPost  : crée backup_YYYY-MM-DD_HH-MM-SS.json, garde max 25 fichiers
...
export async function copyCodeGs(btnEl) {
  try {
    await navigator.clipboard.writeText(CODE_GS);
    btnEl.textContent = '✅ Copié !';
    setTimeout(() => { btnEl.textContent = '📋 Copier le script ici'; }, 2000);
  } catch {
    btnEl.textContent = '❌ Erreur presse-papier';
  }
}

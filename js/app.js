...
let _currentCleanup = null; // Fonction de nettoyage de la page actuelle
...
export async function navigateTo(page, params = {}) {
  ...
  // Chargement et rendu du module de page
  const content = document.getElementById('app-content');
  content.innerHTML = '<div class="loading"><div class="spinner"></div></div>';
...
async function checkUnfilledMonths() {
  try {
    ...
    // Notification si mois courant pas rempli
    const settings = await getAllSettings();
    if (settings.notifEnabled && isMonthEmpty(monthMap[month])) {
      if ('Notification' in window && Notification.permission === 'granted') {
        new Notification('Budget Foyer', {
...
async function init() {
  ...
  try {
    State.settings = await getAllSettings();
    State.p1Name   = State.settings.p1Name || 'Personne 1';
    State.p2Name   = State.settings.p2Name || 'Personne 2';
  } catch (e) {
    console.warn('[App] Impossible de charger les réglages :', e);
  }
  ...
  // Gestion du hash pour deep-link (ex: ?page=saisie)
  const hash = window.location.hash.replace('#', '').trim();
  const urlPage = hash && PAGES[hash] ? hash : 'dashboard';
  ...
  // Vérification des mois non remplis (en arrière-plan)
  setTimeout(checkUnfilledMonths, 1500);
...
// ── Rechargement des noms (après modification dans réglages) ──
export async function reloadNames() {
  State.p1Name = await getSetting('p1Name') || 'Personne 1';
  State.p2Name = await getSetting('p2Name') || 'Personne 2';
}
